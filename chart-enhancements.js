(function(){
  ...
  window.renderTrendChart=function(){
    ...
    const changeLabels=hasData?data.map((r,i)=>{if(i===0)return '';const ch=pct(Number(r.income)||0,Number(data[i-1].income)||0);if(ch===null)return '';return `<text x="${rightX+10}" y="${y(performance(r))+4}" class="chart-axis-label chart-change-axis ${ch>=0?'up':'down'}">${pctText(ch)}</text>`}).join(''):'';
    ...
    const candles=hasData?data.map((r,i)=>{if(i===0)return '';const cur=Number(r.income)||0,prev=Number(data[i-1].income)||0,ch=pct(cur,prev)||0;const y1=y(performance(data[i-1])),y2=y(performance(r)),top=Math.min(y1,y2),height=Math.max(16,Math.abs(y2-y1)),bodyW=20;return `<g data-candle="${i}" class="chart-candle-group ${ch>=0?'up':'down'}"><line x1="${x(i)}" y1="${top-9}" x2="${x(i)}" y2="${top+height+9}" class="chart-candle-wick"/><rect x="${x(i)-bodyW/2}" y="${top}" width="${bodyW}" height="${height}" rx="4" class="chart-candle-body"/><line x1="${x(i)-13}" y1="${y2}" x2="${x(i)+13}" y2="${y2}" class="chart-candle-cap"/></g>`}).join(''):'';
    ...
    applyPlotZoom(el,pad,chartState.zoom,x);
  };
  ...
  function bindInteractions(el,data,w,h,pad,x,y,performance){
    ...
    const move=e=>{
      ...
      const i=Math.max(0,Math.min(data.length-1,Math.round((px-pad.l-18)/(Math.max(58,(w-pad.l-pad.r)/Math.min(data.length,10))*chartState.zoom))));
