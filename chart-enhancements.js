(function(){
  const style=document.createElement('style');style.textContent='.chart-y-axis-left,.chart-y-axis-right{position:absolute;top:0;bottom:0;width:58px;pointer-events:none;z-index:4;font:700 10px Inter,system-ui,sans-serif}.chart-y-axis-left{left:0}.chart-y-axis-right{right:0;text-align:left}.chart-y-axis-left span,.chart-y-axis-right span{position:absolute;transform:translateY(-50%);white-space:nowrap}.chart-y-axis-left span{right:8px;color:#7189a2}.chart-y-axis-right span{left:8px}.chart-y-axis-right .up{color:#55dfab}.chart-y-axis-right .down{color:#ff9c88}.chart-stage .chart-axis-line{pointer-events:none}.chart-candles{opacity:1}.chart-candle-wick,.chart-candle-cap{display:none}.chart-candle-body{stroke-width:2}.chart-line{display:none}@media(max-width:650px){.chart-y-axis-left,.chart-y-axis-right{width:45px}.chart-y-axis-left span{right:5px}.chart-y-axis-right span{left:5px}}';document.head.appendChild(style);
  ...
  window.renderTrendChart=function(){
    ...
    const w=1000,h=390,pad={l:78,r:78,t:28,b:68},innerW=w-pad.l-pad.r,innerH=h-pad.t-pad.b;
    const baseIncome=hasData?(Number(data[0].income)||0):0,baseProfit=hasData?(Number(data[0].profit)||0):0;
    ...
    const perf=data.map(performance),maxPerf=hasData?Math.max(100,...perf):100,minPerf=0,maxY=Math.ceil((maxPerf+10)/10)*10,minY=0,range=Math.max(100,maxY-minY);
    const y=v=>pad.t+(maxY-v)*innerH/range;
    const zeroY=y(0),baseStep=data.length>1?Math.max(62,innerW/Math.min(data.length,9)):72;
    const x=i=>pad.l+20+i*baseStep*chartState.zoom,rightX=w-pad.r;
    ...
    // Clean vertical business candles: no wick/tail. Every candle grows upward from the baseline.
    const candles=hasData?data.map((r,i)=>{
      const cur=Number(r.income)||0,prev=i>0?(Number(data[i-1].income)||0):cur;
      const ch=i===0?0:pct(cur,prev)||0;
      const value=Math.max(0,performance(r));
      const top=Math.max(pad.t,y(value));
      const height=Math.max(8,zeroY-top);
      const bodyW=30;
      return `<g data-candle="${i}" class="chart-candle-group ${ch>=0?'up':'down'}"><rect x="${x(i)-bodyW/2}" y="${top}" width="${bodyW}" height="${height}" rx="5" class="chart-candle-body"/></g>`;
    }).join(''):'';
  ...
  function applyPlotZoom(el,pad,zoom,x){
    const group=el.querySelector('[data-plot-group]');
    if(group)group.setAttribute('transform',`translate(${pad.l},0) scale(${zoom},1) translate(${-pad.l},0)`);
