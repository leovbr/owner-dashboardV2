(function(){
  ...
  window.renderTrendChart=function(){
    ...
    const visibleWidth=Math.min(w-pad.r,pad.l+plotW);
    ...
    const tickCount=5;
    const grid=Array.from({length:tickCount},(_,i)=>{const v=maxY-range*(i/(tickCount-1));return `<line x1="${pad.l}" y1="${y(v)}" x2="${visibleWidth}" y2="${y(v)}" class="chart-grid-line"/><text x="${pad.l-12}" y="${y(v)+4}" text-anchor="end" class="chart-axis-label">${Math.round(v)}%</text>`}).join('');
    const rightTicks=Array.from({length:tickCount},(_,i)=>{const v=maxY-range*(i/(tickCount-1));const idx=Math.round((i/(tickCount-1))*(data.length-1));const cur=Number(data[idx]?.income)||0,prev=idx>0?(Number(data[idx-1].income)||0):0;const ch=idx>0?pct(cur,prev):0;return `<text x="${visibleWidth+12}" y="${y(v)+4}" text-anchor="start" class="chart-axis-label chart-right-pct">${idx===0?'0%':pctText(ch)}</text>`}).join('');
    const labels=data.map((r,i)=>{if(!hasData)return '';if(data.length>12&&i%Math.ceil(data.length/9)!==0&&i!==data.length-1)return '';return `<text x="${x(i)}" y="${h-20}" text-anchor="middle" class="chart-label">${dateLabel(r.date)}</text>`}).join('');
    const dots=(cls,fn)=>data.map((r,i)=>`<circle cx="${x(i)}" cy="${y(fn(r))}" r="5" class="chart-data-dot ${cls}"/>`).join('');
    const candleBars=hasData?data.map((r,i)=>{if(i===0)return '';const prev=Number(data[i-1].income)||0,cur=Number(r.income)||0,ch=pct(cur,prev);const barH=Math.max(10,Math.abs(y(performance(r))-y(performance(data[i-1]))));const top=Math.min(y(performance(r)),y(performance(data[i-1])));return `<line x1="${x(i)}" y1="${top}" x2="${x(i)}" y2="${top+barH}" class="chart-candle ${ch>=0?'up':'down'}"/><line x1="${x(i)-7}" y1="${y(performance(r))}" x2="${x(i)+7}" y2="${y(performance(r))}" class="chart-candle-cap ${ch>=0?'up':'down'}"/>`}).join(''):'';
    const last=hasData?data[data.length-1]:null,prev=hasData&&data.length>1?data[data.length-2]:null,lastChange=prev?pct(Number(last.income)||0,Number(prev.income)||0):0,lastPerf=hasData?performance(last):0;
    el.innerHTML=`<div class="chart-stage" data-chart-stage><svg viewBox="0 0 ${w} ${h}" preserveAspectRatio="none" role="img" aria-label="Grafik performa dan perubahan omzet"><g class="chart-grid-layer ${chartState.grid?'visible':''}">${grid}</g><line x1="${pad.l}" y1="${pad.t}" x2="${pad.l}" y2="${h-pad.b}" class="chart-axis-line"/><line x1="${visibleWidth}" y1="${pad.t}" x2="${visibleWidth}" y2="${h-pad.b}" class="chart-axis-line"/><line x1="${pad.l}" y1="${h-pad.b}" x2="${visibleWidth}" y2="${h-pad.b}" class="chart-axis-line"/><g class="chart-right-labels">${rightTicks}</g><g class="chart-candles">${candleBars}</g><polyline points="${incomePoints}" class="chart-line chart-income"/><polyline points="${profitPoints}" class="chart-line chart-profit"/><g class="chart-dots-layer ${chartState.dots?'visible':''}">${dots('chart-income-dot',performance)}${dots('chart-profit-dot',profitPerf)}</g><g>${labels}</g><g class="chart-crosshair-layer ${chartState.crosshair?'visible':''}"><line data-crosshair-x x1="${pad.l}" y1="${pad.t}" x2="${pad.l}" y2="${h-pad.b}" class="chart-crosshair-x"/><line data-crosshair-y x1="${pad.l}" y1="${y(lastPerf)}" x2="${visibleWidth}" y2="${y(lastPerf)}" class="chart-crosshair-y"/><circle data-crosshair-dot cx="${pad.l}" cy="${y(lastPerf)}" r="7" class="chart-crosshair-dot"/></g></svg><div class="chart-tooltip" data-chart-tooltip hidden></div></div><div class="chart-bottom"><div class="chart-legend"><span><i class="legend-income"></i> Omzet</span><span><i class="legend-profit"></i> Keuntungan</span>${hasData&&prev?`<span class="chart-change ${lastChange>=0?'up':'down'}">${pctText(lastChange)}</span>`:'<span class="chart-last-readout">Tambahkan laporan harian untuk melihat perubahan</span>'}</div><div class="chart-tools"><button type="button" class="chart-toggle ${chartState.grid?'on':''}" data-chart-toggle="grid">▦ Grid</button><button type="button" class="chart-toggle ${chartState.crosshair?'on':''}" data-chart-toggle="crosshair">＋ Garis</button><button type="button" class="chart-toggle ${chartState.dots?'on':''}" data-chart-toggle="dots">● Titik</button></div></div>`;
  ...
  function bindInteractions(el,data,w,h,pad,x,y,performance){
    ...
    const move=e=>{
      ...
      const rect=svg.getBoundingClientRect();let px=(e.clientX-rect.left)/rect.width*w;px=clamp(px,pad.l,w-pad.r);const ratio=clamp((px-pad.l)/(w-pad.l-pad.r),0,1),i=Math.max(0,Math.min(data.length-1,Math.round(ratio*(data.length-1))));
      const sx=x(i),income=Number(data[i].income)||0,profit=Number(data[i].profit)||0,prev=i>0?(Number(data[i-1].income)||0):0,ch=i>0?pct(income,prev):0,crossX=stage.querySelector('[data-crosshair-x]'),crossY=stage.querySelector('[data-crosshair-y]'),dot=stage.querySelector('[data-crosshair-dot]');
    ...
    stage.addEventListener('touchmove',e=>{if(e.touches.length===2&&pinchStart){const dist=Math.abs(e.touches[0].clientX-e.touches[1].clientX);const factor=dist/pinchStart;chartState.zoom=clamp(chartState.zoom*factor,.75,3);pinchStart=dist;svg.style.width=`${chartState.zoom*100}%`;svg.style.maxWidth='none';e.preventDefault();}},{passive:false});
